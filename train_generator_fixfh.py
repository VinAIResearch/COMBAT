...
def get_model(opt):
    ...
    return netC, netG, optimizerG, schedulerG, netF, netF_eval, clean_model
...
def train(
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        progress_bar(
            batch_idx,
            len(train_dl),
            "Clean Acc: {:.4f} | Bd Acc: {:.4f} | F Acc: {:.4f} | Clean Model Acc: {:.4f} | Clean Model Bd Acc: {:.4f} | Clean Model Bd ASR: {:.4f}".format(
                avg_acc_clean,
                avg_acc_bd,
                avg_acc_F,
                avg_clean_model_acc,
...
def main():
    ...
    # prepare model
    netC, netG, optimizerG, schedulerG, netF, netF_eval, clean_model = get_model(opt)
    ...
    if opt.continue_training:
        if os.path.exists(opt.ckpt_path):
            ...
            best_clean_model_asr = state_dict["best_clean_model_bd_ba"]
            best_clean_model_asr = state_dict["best_clean_model_bd_asr"]
