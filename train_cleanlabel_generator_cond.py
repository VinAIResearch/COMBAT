...
def train(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, train_dl, mask, pattern, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        loss_ce += 50 * criterion_CE(pred_bd, tmp)
        ...
        progress_bar(batch_idx, len(train_dl), 'CE Loss: {:.4f} | L2 Loss: {:.6f} | Clean Acc: {:.4f} | Bd Acc: {:.4f}'.format(avg_loss_ce, avg_loss_l2,
                                                                                                            avg_acc_clean,
                                                                                                            avg_acc_bd))
    ...
    # for tensorboard
    if(not epoch % 1):
        tf_writer.add_scalars('Clean Accuracy', {'Clean': avg_acc_clean, 'Bd': avg_acc_bd, 'L2' : avg_loss_l2}, epoch)
        tf_writer.add_image('Images', grid, global_step=epoch)
        
    schedulerC.step()        


def eval(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, test_dl, mask, pattern, best_clean_acc, best_bd_acc, tf_writer, epoch, opt):
    print(" Eval:")
    netC.eval()
...
def main():
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        train(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, train_dl, mask, pattern, tf_writer, epoch, opt)
