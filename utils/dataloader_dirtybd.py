...
class GTSRB(data.Dataset):
    def __init__(self, opt, train, transforms):
        ...
        if train:
            ...
            self.images, self.labels = self._get_data_train_list(target_label, opt.pc)
        else:
            self.data_folder = os.path.join(opt.data_root, "GTSRB/Test")
            self.images, self.labels = self._get_data_test_list(target_label, opt.pc)
...
class PoisonedDataset(data.Dataset):
    ...
    def _poison_flags(self, target_label, pc):
        num_poisoned = max(0, int(pc * len(self.dataset)))
        print(f"Poison {num_poisoned} images ({pc * len(self.dataset)})")
        poisoned = set(random.sample(range(len(self.dataset)), num_poisoned))
        return poisoned
...
def get_dataloader(opt, train=True, pretensor_transform=False, bs=None, shuffle=True):
    ...
    if opt.dataset == "gtsrb":
        dataset = PoisonedDataset(GTSRB(opt, train, transform))
    elif opt.dataset == "gtsrb2":
        dataset = PoisonedDataset(GTSRB2(opt, train, transform))
    elif opt.dataset == "mnist":
