...
def eval(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, test_dl, mask, pattern, best_clean_acc, best_bd_acc, best_F_acc, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_dl):
        with torch.no_grad():
            ...
            acc_F = total_F_correct * 100.0 / (total_sample * opt.num_classes)
    ...
    if acc_clean > best_clean_acc:
        ...
        best_bd_acc = acc_bd
        best_F_acc = acc_F
        state_dict = {"netC": netC.state_dict(), "schedulerC": schedulerC.state_dict(), "optimizerC": optimizerC.state_dict(), "netG": netG.state_dict(), "schedulerG": schedulerG.state_dict(), "optimizerG": optimizerG.state_dict(), "best_clean_acc": acc_clean, "best_bd_acc": acc_bd, "best_F_acc": acc_F, "epoch_current": epoch, "mask": mask, "pattern": pattern}
...
def main():
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        best_clean_acc, best_bd_acc, best_F_acc = eval(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, test_dl, mask, pattern, best_clean_acc, best_bd_acc, best_F_acc, tf_writer, epoch, opt)
