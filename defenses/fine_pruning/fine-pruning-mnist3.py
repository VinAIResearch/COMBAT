...
def main():
    ...
    if(opt.dataset == 'cifar10'):
        ...
        netC = NetC_MNIST3().to(opt.device) #PreActResNet10(n_input=1).to(opt.device)
    else:
        raise Exception("Invalid dataset")
    ...
    with open(opt.outfile, 'w') as outs:
      for index in range(pruning_mask.shape[0]):
        ...
        net_pruned.layer3.conv1 = nn.Conv2d(pruning_mask.shape[0], pruning_mask.shape[0] - num_pruned, (3, 3), stride=2, padding=1, bias=False)
