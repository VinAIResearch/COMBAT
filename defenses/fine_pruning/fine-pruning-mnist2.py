...
from networks.models import AE, Normalizer, Denormalizer, NetC_MNIST, NetC_MNIST2
...
def main():
    ...
    if(opt.dataset == 'cifar10'):
        ...
        netC = NetC_MNIST2().to(opt.device) #PreActResNet10(n_input=1).to(opt.device)
    else:
        raise Exception("Invalid dataset")
    ...
    with open(opt.outfile, 'w') as outs:
      for index in range(pruning_mask.shape[0]):
        ...
        net_pruned.layer4.conv1 = nn.Conv2d(pruning_mask.shape[0], pruning_mask.shape[0] - num_pruned, (3, 3), stride=2, padding=1, bias=False)
        ...
        for name, module in net_pruned._modules.items():
            if('layer4' in name):
                module.conv1.weight.data = netC.layer4.conv1.weight.data[pruning_mask]
                module.ind = pruning_mask
            elif('linear6' == name):
                module.weight.data = netC.linear6.weight.data.reshape(-1, 64, 16)[:, pruning_mask].reshape(512, -1) #[:, pruning_mask]
