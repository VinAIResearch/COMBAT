...
def train(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, train_dl, train_dl2, mask, pattern, tf_writer, epoch, opt):
    ...
    for batch_idx, batch1, batch2 in zip(range(l), train_dl, train_dl2):
        ...
        loss = loss_ce + opt.cross_weight * loss_cross + opt.L2_weight * loss_l2 + opt.F_weight * loss_F
        loss.backward()
        optimizerG.step()
        ...
        progress_bar(
            batch_idx,
            len(train_dl),
            "CE Loss: {:.4f} | L2 Loss: {:.6f} | F Loss: {:.6f} | Clean Acc: {:.4f} | Bd Acc: {:.4f} | Cross Acc: {:.4f} | F Acc: {:.4f}".format(
                avg_loss_ce,
                avg_loss_l2,
                # avg_loss_grad_l2,
                avg_loss_F,
    ...
    # for tensorboard
    if not epoch % 1:
        tf_writer.add_scalars("Clean Accuracy", {"Clean": avg_acc_clean, "Bd": avg_acc_bd, "Cross": avg_acc_cross, "F": avg_acc_F, "L2 Loss": avg_loss_l2, "F Loss": avg_loss_F}, epoch)
...
def eval(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, test_dl, test_dl2, mask, pattern, best_clean_acc, best_bd_acc, best_cross_acc, best_F_acc, tf_writer, epoch, opt):
    print(" Eval:")
    netC.eval()
    ...
    # tensorboard
    if not epoch % 1:
        tf_writer.add_scalars("Test Accuracy", {"Clean": acc_clean, "Bd": acc_bd, "Cross": acc_cross, "F": acc_F}, epoch)
    ...
    if acc_clean > best_clean_acc:
        ...
        best_F_acc = acc_F
        state_dict = {"netC": netC.state_dict(), "schedulerC": schedulerC.state_dict(), "optimizerC": optimizerC.state_dict(), "netG": netG.state_dict(), "schedulerG": schedulerG.state_dict(), "optimizerG": optimizerG.state_dict(), "best_clean_acc": acc_clean, "best_bd_acc": acc_bd, "best_cross_acc": acc_cross, "best_F_acc": acc_F, "epoch_current": epoch, "mask": mask, "pattern": pattern}
        torch.save(state_dict, opt.ckpt_path)
    return best_clean_acc, best_bd_acc, best_cross_acc, best_F_acc
...
def main():
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        train(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, train_dl, train_dl2, mask, pattern, tf_writer, epoch, opt)
        best_clean_acc, best_bd_acc, best_cross_acc, best_F_acc = eval(netC, optimizerC, schedulerC, netG, optimizerG, schedulerG, netF, test_dl, test_dl2, mask, pattern, best_clean_acc, best_bd_acc, best_cross_acc, best_F_acc, tf_writer, epoch, opt)
